...
export async function POST(req: Request) {
  ...
  try {
    const user = isRenewingPassword
      ? // User is renewing password
        await client.users.findFirst({
          where: { shms_email: userEmail, shms_user_is_deleted: false }
        })
      : // Check for user with the reset token and ensure the user is not deleted
        await client.users.findFirst({
          where: { shms_user_reset_token: resetToken, shms_user_is_deleted: false }
        })
    ...
    if (!user) {
      ...
      // Send the user an email with a link to sign in to their account
