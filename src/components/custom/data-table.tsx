...
export default function OperationsTable({
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (paginationRef.current && tableContainerRef.current) {
        const containerRect = tableContainerRef.current.getBoundingClientRect();
        const paginationRect = paginationRef.current.getBoundingClientRect();
        const shouldStick =
          containerRect.top <= 0 && containerRect.bottom > paginationRect.height;
  ...
  return (
    <div ref={tableContainerRef} className="w-full rtl">
      ...
      <div
        ...
        ${isSticky ? "sticky left-0 right-0 border bg-card z-10 shadow px-4" : ""}`}
        style={
          isSticky
            ? { top: "0", width: `${tableContainerRef.current?.offsetWidth}px` }
            : {}
        }
      >
